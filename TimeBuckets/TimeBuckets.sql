...
IF OBJECT_ID('tempdb..#DaysOfWeek') IS NOT NULL
	...
	SELECT @StarDate  = TRY_CONVERT(DATETIME,TRY_CONVERT(VARCHAR(4),DATEPART(YY, @StarDate))+'-01-01');
	...
	,StartDate
	,EndDate
	,CASE WHEN RIGHT('00'+TRY_CONVERT(VARCHAR(50),DATEPART(WEEK, StartDate)),2) = 53 THEN 1 ELSE RIGHT('00'+TRY_CONVERT(VARCHAR(50),DATEPART(WEEK, StartDate)),2) END	AS 'Week'
	,MIN(StartDate) OVER (PARTITION BY TRY_CONVERT(VARCHAR(50), DATEPART(YY, StartDate))+'_'+TRY_CONVERT(VARCHAR(50), RIGHT('00'+TRY_CONVERT(VARCHAR(50),DATEPART(WEEK, StartDate)),2)))
	,MAX(EndDate) OVER (PARTITION BY TRY_CONVERT(VARCHAR(50), DATEPART(YY, StartDate))+'_'+TRY_CONVERT(VARCHAR(50), RIGHT('00'+TRY_CONVERT(VARCHAR(50),DATEPART(WEEK, StartDate)),2)))
